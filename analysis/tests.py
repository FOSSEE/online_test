...
from yaksh.models import (
		Profile, Course, Question, Quiz, LearningUnit, 
		LearningModule, QuestionPaper, AnswerPaper,
		Answer
	)
import pytz
...
class ModeratorAnalysisTest(TestCase):
	...
	def test_moderator_can_view_moderator_analysis(self):
		...
		login = client.login(username=self.user_teacher.username, password=self.user_teacher_pass)
		response = client.get(reverse('analysis:final_summary'))
...
class StudentAnalysisTest(TestCase):
	...
	def create_course(self):
		self.course = Course.objects.create(
						name='demo_course', 
						enrollment="open", 
						start_enroll_time = datetime(2018, 6, 29, 13, 49, 0, 0, tzinfo=pytz.UTC),
					  	end_enroll_time=datetime(2019, 6, 29, 13, 49, 0, 0, tzinfo=pytz.UTC),
	...
	def test_char_age_value(self):
		...
		client.post(reverse('yaksh:register'),
							data={'username': 'demo_user',
							  ...
							  'institute': 'demo_institute', 'department': 'demo_dept',
							  'position': 'Graduate Student', 'timezone': pytz.utc.zone,
							  'age': 'eighteen', 'gender': 'Male', 'state': 'Gujarat'
							  }
							)
