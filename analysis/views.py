...
def make_group(data, catt):
    ...
    group = nested_dict(6)
    grouped = data.pivot_table(columns=['course_group', 'course_tag', 'year', 'month'],
                               index=catt, values='course_id',
                               aggfunc='count',
                               fill_value=0).unstack().reset_index().rename(columns={0: 'count'})
    for index, row in grouped.iterrows():
        group[row['course_group']][row['course_tag']][catt][row['year']
                                                            ][row['month']][row[catt]] = row['count']

    grouped_all = data.pivot_table(columns=['course_tag', 'year', 'month'],
                                   index=catt, values='course_id',
                                   aggfunc='count',
                                   fill_value=0).unstack().reset_index().rename(columns={0: 'count'})
    for index, row in grouped_all.iterrows():
        group['All'][row['course_tag']][catt][row['year']
                                              ][row['month']][row[catt]] = row['count']
    ...
    if catt == 'grade':
        ...
        for index, row in grouped.iterrows():
            if 'Passed' not in group[row['course_group']][row['course_tag']][catt][row['year']][row['month']]:
                group[row['course_group']][row['course_tag']
                                           ][catt][row['year']][row['month']]['Passed'] = 0
            if row[catt] in PASS_GRADES:
                group[row['course_group']][row['course_tag']
                                           ][catt][row['year']][row['month']]['Passed'] += row['count']
        ...
        for index, row in grouped_all.iterrows():
            ...
            if row[catt] in PASS_GRADES:
                group['All'][row['course_tag']][catt][row['year']
                                                      ][row['month']]['Passed'] += row['count']

    # enrolled count
    counts = data.groupby(['course_group', 'course_tag', 'year', 'month'])[
        'month'].agg(['count']).reset_index()
    for index, row in counts.iterrows():
        group[row['course_group']][row['course_tag']][catt][row['year']
                                                            ][row['month']]['Enrolled'] = row['count']

    counts_all = data.groupby(['course_tag', 'year', 'month'])[
        'month'].agg(['count']).reset_index()
...
def get_enroll_count_perlevel(data, final):
    counts = data.groupby(['course_group', 'course_tag'])[
        'course_tag'].agg(['count']).reset_index()
    for index, row in counts.iterrows():
        final[row['course_group']][row['course_tag']]['Enrolled'] = row['count']

    counts_all = data.groupby(['course_tag'])[
        'course_tag'].agg(['count']).reset_index()
    for index, row in counts_all.iterrows():
...
def get_grade_count_perlevel(data, final, grades):
    ...
    counts = data.groupby(['course_group', 'course_tag', 'grade'])[
        'grade'].agg(['count']).reset_index()
    for index, row in counts.iterrows():
        if row['grade'] in grades:
            final[row['course_group']][row['course_tag']
                                       ][row['grade']] = row['count']
    ...
    counts_all = data.groupby(['course_tag', 'grade'])[
        'grade'].agg(['count']).reset_index()
    for index, row in counts_all.iterrows():
        if row['grade'] in grades:
    ...
    for i, group in enumerate(groups):
        for group_key in group_keys:
            for level_key in level_keys:
                final[group_key][level_key][catts[i]
                                            ] = group[group_key][level_key][
                                                catts[i]]
...
def get_data(start_date, end_date):
    ...
    data = list(CourseStatus.objects
                .filter(course__start_enroll_time__range=(start_date, end_date))
                .values('grade', 'course_id',
                        'user__profile__institute',
                        'user__profile__position',
                        'user__profile__gender',
                        'user__profile__age',
                        'user__profile__state',
                        'course__name', 'course__level',
                        'course__group',
    ...
    data.rename(columns={'user__profile__institute': 'institute',
                         ...
                         'course__start_enroll_time':
                         'course_start_enroll_time'},
                inplace=True)

    if data.empty:
        return {}

    labels = [0, '1-15', '16-25', '26-35', '36-45',
              '46-55', '56-65', '66-75', '76-85']
    data['user_age_group'] = pd.cut(data['user_age'], [
                                    0, 1, 15, 25, 35, 45, 55, 65, 75, 85],
                                    labels=labels, include_lowest=True)

    months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June',
              'July', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec']
    data['month'] = data['course_start_enroll_time'].apply(
        lambda row: months[row.month - 1])
    data['year'] = data['course_start_enroll_time'].apply(
        lambda row: str(row.year))

    coursewise = merge(data, ['grade', 'user_gender',
                              'user_position', 'user_age_group'])

    statewise = nested_dict(2)

    enrolled = data.groupby(['course_group', 'state'])[
        'state'].agg(['count']).reset_index()
    for index, row in enrolled.iterrows():
        statewise[row['course_group']][row['state']] = row['count']

    enrolled_all = data.groupby(['state'])['state'].agg([
        'count']).reset_index()
    ...
    enrolled = data.groupby(['course_group', 'institute'])[
        'institute'].agg(['count']).reset_index()
    for index, row in enrolled.iterrows():
        institutewise[row['course_group']
                      ][row['institute']]['Enrolled'] = row['count']

    # passed per institute in groups
    passed = data[data['grade'] != 'F'].groupby(['course_group', 'institute'])[
        'institute'].agg(['count']).reset_index()
    for index, row in passed.iterrows():
        institutewise[row['course_group']
                      ][row['institute']]['Passed'] = row['count']

    # enrolled per institute per courese in groups
    enrolled_percourse = data.groupby(['course_group', 'institute', 'course_name'])[
        'course_name'].agg(['count']).reset_index()
    for index, row in enrolled_percourse.iterrows():
        institutewise[row['course_group']][row['institute']
                                           ][row['course_name']]['Enrolled'] = row['count']

    # passed per institute per courese in groups
    passed_percourse = data[data['grade'] != 'F'].groupby(
        ['course_group', 'institute', 'course_name'])['course_name'].agg(['count']).reset_index()
    for index, row in passed_percourse.iterrows():
        institutewise[row['course_group']][row['institute']
                                           ][row['course_name']]['Passed'] = row['count']

    # enrolled per institute all
    enrolled_all = data.groupby(['institute'])[
        'institute'].agg(['count']).reset_index()
    for index, row in enrolled_all.iterrows():
        institutewise['All'][row['institute']]['Enrolled'] = row['count']

    # passed per institute all
    passed_all = data[data['grade'] != 'F'].groupby(
        ['institute'])['institute'].agg(['count']).reset_index()
    ...
    enrolled_all_percourse = data.groupby(['institute', 'course_name'])[
        'course_name'].agg(['count']).reset_index()
    for index, row in enrolled_all_percourse.iterrows():
        institutewise['All'][row['institute']
                             ][row['course_name']]['Enrolled'] = row['count']

    # passed per institute per courese all
    passed_all_percourse = data[data['grade'] != 'F'].groupby(
        ['institute', 'course_name'])['course_name'].agg(['count']).reset_index()
...
def view_quiz_stats(request, course_id, question_paper_id):
    ...
    # print(que_stats)
    if questionpaper.quiz.has_code_questions() \
            and user in course.students.all():

        return render(request, 'view_quiz_stats.html',
